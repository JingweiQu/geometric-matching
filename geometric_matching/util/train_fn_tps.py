...
def train_fn(epoch, model, loss_fn, loss_cycle_fn, loss_jitter_fn, optimizer, dataloader, triple_generation,
             geometric_model='tps', use_cuda=True, log_interval=100, vis=None, show=False):
    """
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss = loss_match + (loss_cycle_st + loss_cycle_ts + loss_cycle_tr + loss_cycle_rt) / 4 + loss_jitter
        loss.backward()
        optimizer.step()
