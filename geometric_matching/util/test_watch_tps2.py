...
def test_watch(model=None, faster_rcnn=None, aff_theta=None, metric='pck', batch_size=1, dataset=None, dataloader=None, detect=False, dual=False, do_aff=False, do_tps=False, args=None):
    ...
    if do_tps:
        results['tps'] = {}
        theta['tps'] = torch.Tensor(N, 24).cuda()
        thetai['tps'] = torch.Tensor(N, 24).cuda()
    if dual:
        results['aff'] = {}
        results['afftps'] = {}
        theta['aff'] = torch.Tensor(N, 6).cuda()
        theta['afftps'] = torch.Tensor(N, 24).cuda()
        thetai['aff'] = torch.Tensor(N, 6).cuda()
        thetai['afftps'] = torch.Tensor(N, 24).cuda()
    if detect:
        results['det'] = {}
        results['det_aff']={}
        results['det_aff_tps'] = {}
        ...
        theta['det_aff_tps'] = torch.Tensor(N, 24).cuda()
        thetai['det'] = torch.Tensor(N, 6).cuda()
        thetai['det_aff'] = torch.Tensor(N, 6).cuda()
        thetai['det_aff_tps'] = torch.Tensor(N, 24).cuda()
