    ...
    for key in stats.keys():
        ...
        for metric in metrics:
            ...
            if isinstance(dataset, PFPASCALDataset):
                N_cat = int(np.max(dataset.category))   # Number of categories in dataset (PF-PASCAL)
                for c in range(N_cat):
                    cat_idx = np.nonzero(dataset.category == c + 1)[0]  # Compute indices of current category
                    print(dataset.category_names[c].ljust(15) + ': ',
