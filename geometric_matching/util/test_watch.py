...
from geometric_matching.geotnf.flow import th_sampling_grid_to_np_flow, write_flo_file, visualize_flow
...
def test_watch(model=None, faster_rcnn=None, aff_theta=None, metric='pck', batch_size=1, dataset=None, dataloader=None, detect=False, dual=False, do_aff=False, do_tps=False, args=None):
    # Initialize results
    N = len(dataset)
    results = {}
    theta = {}
    ...
    if do_aff:
        results['aff'] = {}
        theta['aff'] = torch.Tensor(N, 6).cuda()
        thetai['aff'] = torch.Tensor(N, 6).cuda()
    if do_tps:
        results['tps'] = {}
        theta['tps'] = torch.Tensor(N, 18).cuda()
        thetai['tps'] = torch.Tensor(N, 18).cuda()
    if dual:
        results['aff'] = {}
        results['afftps'] = {}
        theta['aff'] = torch.Tensor(N, 6).cuda()
        theta['afftps'] = torch.Tensor(N, 18).cuda()
        thetai['aff'] = torch.Tensor(N, 6).cuda()
        thetai['afftps'] = torch.Tensor(N, 18).cuda()
    if detect:
        results['det'] = {}
        results['det_aff']={}
        results['det_aff_tps'] = {}
        theta['det'] = torch.Tensor(N, 6).cuda()
        theta['det_aff'] = torch.Tensor(N, 6).cuda()
        theta['det_aff_tps'] = torch.Tensor(N, 18).cuda()
        thetai['det'] = torch.Tensor(N, 6).cuda()
        thetai['det_aff'] = torch.Tensor(N, 6).cuda()
        thetai['det_aff_tps'] = torch.Tensor(N, 18).cuda()
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        batch_inver = {'source_image': batch['target_image'], 'target_image': batch['source_image']}
        ...
        if detect:
            ...
            theta['det'][indices] = theta_det.view(-1, 6)
            theta['det_aff'][indices] = theta_aff
            theta['det_aff_tps'][indices] = theta_afftps
            # Inverse warping
            theta_det_inver = aff_theta(boxes_s=box_t, boxes_t=box_s)
            thetai_afftps, thetai_aff = model(batch_inver, theta_det_inver)
            thetai['det'][indices] = theta_det_inver.view(-1, 6)
            thetai['det_aff'][indices] = thetai_aff
            thetai['det_aff_tps'][indices] = thetai_afftps
        if dual:
            theta_afftps, theta_aff = model(batch)
            theta['aff'][indices] = theta_aff
            theta['afftps'][indices] = theta_afftps

            thetai_afftps, thetai_aff = model(batch_inver)
            thetai['aff'][indices] = thetai_aff
            thetai['afftps'][indices] = thetai_afftps
        if do_aff:
            theta_aff = model(batch)
            theta['aff'][indices] = theta_aff

            thetai_aff = model(batch_inver)
            thetai['aff'][indices] = thetai_aff
        if do_tps:
            theta_tps = model(batch)
            theta['tps'][indices] = theta_tps

            thetai_tps = model(batch_inver)
            thetai['tps'][indices] = thetai_tps
    ...
    if metric == 'flow':
        ...
        return results, theta, thetai, end - begin
    ...
    for key in results.keys():
        ...
        for metric in metrics:
            ...
            for idx in range(len(good_idx)):
                print('Example {}: {:.4}'.format(idx+1, float(filtered_values[idx])))
...
def point_dist_metric(batch,batch_start_idx,theta_aff,theta_tps,theta_afftps,results,args,use_cuda=True):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_afftps is not None
    ...
    if do_aff_tps:
        # do tps+affine
        warped_points_aff_tps_norm = pt.tpsPointTnf(theta_afftps,target_points_norm)
...
#    import pdb; pdb.set_trace()
    ...
    if do_aff_tps:
        results['afftps']['dist'][indices] = dist_aff_tps.unsqueeze(1).cpu().numpy() 

    return results

def pascal_parts_metrics(batch,batch_start_idx,theta_aff,theta_tps,theta_afftps,results,args,use_cuda=True):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_afftps is not None
    ...
    for b in range(batch_size):
        ...
        if batch['keypoint_A'][b].size!=0:
            ...
            results = pck_metric(batch_b,
                               ...
                               theta_afftps[b,:].unsqueeze(0) if do_aff_tps else None,
        ...
        grid_aff,grid_tps,grid_aff_tps=theta_to_sampling_grid(h_tgt,w_tgt,
                                                              ...
                                                              theta_afftps[b,:] if do_aff_tps else None,
                                                              use_cuda=use_cuda,
                                                              tps_reg_factor=args.tps_reg_factor)
        ...
        if do_aff_tps:
            warped_mask_aff_tps = F.grid_sample(source_mask, grid_aff_tps)           
            results['afftps']['intersection_over_union'][idx] = intersection_over_union(warped_mask_aff_tps,target_mask)
