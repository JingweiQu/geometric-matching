...
def add_watch(watch_images, image_names, batch_st, batch_tr, affTnf, tpsTnf, geoTnf, theta_aff_tps_st, theta_aff_st, theta_aff_tps_tr, theta_aff_tr, k):
    # warped_image_st = affTnf(batch_st['source_image'][0].unsqueeze(0), theta_aff_st[0].unsqueeze(0))
    # warped_image_st = tpsTnf(warped_image_st, theta_aff_tps_st[0].unsqueeze(0))
    #
    # warped_image_tr = affTnf(batch_tr['source_image'][0].unsqueeze(0), theta_aff_tr[0].unsqueeze(0))
    # warped_image_tr = tpsTnf(warped_image_tr, theta_aff_tps_tr[0].unsqueeze(0))
    #
    # warped_image_sr = affTnf(warped_image_st, theta_aff_tr[0].unsqueeze(0))
    # warped_image_sr = tpsTnf(warped_image_sr, theta_aff_tps_tr[0].unsqueeze(0))

    warped_image_st = geoTnf(batch_st['source_image'][0].unsqueeze(0), theta_aff_st[0].unsqueeze(0), theta_aff_tps_st[0].unsqueeze(0))
    warped_image_tr = geoTnf(batch_tr['source_image'][0].unsqueeze(0), theta_aff_tr[0].unsqueeze(0), theta_aff_tps_tr[0].unsqueeze(0))
    warped_image_sr = geoTnf(warped_image_st, theta_aff_tr[0].unsqueeze(0), theta_aff_tps_tr[0].unsqueeze(0))
...
def train_fn_dual(epoch, model, loss_fn, loss_cycle_fn, optimizer, dataloader, triple_generation, use_cuda=True, log_interval=100, vis=None, show=False):
    """
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        theta_aff_tps_st, theta_aff_st, theta_aff_tps_ts, theta_aff_ts = model(batch_st)  # from source image to target image
        theta_aff_tps_tr, theta_aff_tr, theta_aff_tps_rt, theta_aff_rt = model(batch_tr)  # from target image to refer image
        ...
        loss_match = loss_fn(theta_aff_tps_st=theta_aff_tps_st, theta_aff_st=theta_aff_st, theta_aff_tps_tr=theta_aff_tps_tr,
                             theta_aff_tr=theta_aff_tr, theta_aff_GT=batch_triple['theta_aff_GT'], theta_tps_GT=batch_triple['theta_tps_GT'])
        loss_cycle_st = loss_cycle_fn(theta_aff_tps_st, theta_aff_st, theta_aff_tps_ts, theta_aff_ts)
        loss_cycle_ts = loss_cycle_fn(theta_aff_tps_ts, theta_aff_ts, theta_aff_tps_st, theta_aff_st)
        loss_cycle_tr = loss_cycle_fn(theta_aff_tps_tr, theta_aff_tr, theta_aff_tps_rt, theta_aff_rt)
        loss_cycle_rt = loss_cycle_fn(theta_aff_tps_rt, theta_aff_rt, theta_aff_tps_tr, theta_aff_tr)
        loss = loss_match + (loss_cycle_st + loss_cycle_ts + loss_cycle_tr + loss_cycle_rt) / 4
        loss.backward()
        optimizer.step()
    ...
    if (epoch % 5 == 0 or epoch == 1) and vis is not None:
        ...
        opts_loss = dict(xlabel='Iterations (' + str(stride_loss) + ')',
                         ylabel='Loss',
                         title='GM ResNet101 AffTPSCycle Training Loss in Epoch ' + str(epoch),
                         legend=['Loss'],
                         width=2000)
