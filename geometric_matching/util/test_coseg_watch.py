...
def test_watch(model=None, batch_size=1, dataset=None, dataloader=None, args=None):
    # Initialize results
    N = len(dataset)
    # initialize vector for storing results for each metric
    results = np.zeros((N, 2))
    masks_A = torch.Tensor(N, 1, 240, 240).cuda()
    masks_B = torch.Tensor(N, 1, 240, 240).cuda()
    ...
    return results, masks_A, masks_B
