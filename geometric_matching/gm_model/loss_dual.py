...
from geometric_matching.geotnf.transformation_tps import GeometricTnf, ComposedGeometricTnf
...
class TransformedGridLoss(nn.Module):
    ...
    def forward(self, theta_aff_tps_st, theta_aff_st, theta_aff_tps_tr, theta_aff_tr, theta_aff_GT, theta_tps_GT):
        ...
        batch_size = theta_aff_tps_st.size()[0]
        ...
        P_prime = self.pointTnf.tpsPointTnf(theta_aff_tps_tr.unsqueeze(2).unsqueeze(3), P)
        P_prime = self.pointTnf.affPointTnf(theta_aff_tr, P_prime)
        P_prime = self.pointTnf.tpsPointTnf(theta_aff_tps_st.unsqueeze(2).unsqueeze(3), P_prime)
        P_prime = self.pointTnf.affPointTnf(theta_aff_st, P_prime)
        P_prime_GT = self.pointTnf.tpsPointTnf(theta_tps_GT, P)
        P_prime_GT = self.pointTnf.affPointTnf(theta_aff_GT, P_prime_GT)
...
class CycleLoss(nn.Module):
    ...
    def forward(self, theta_aff_tps_AB, theta_aff_AB, theta_aff_tps_BA, theta_aff_BA):
        # expand grid according to batch size
        # theta.shape: (batch_size, 18) for tps, (batch_size, 6) for affine
        # theta_GT.shape: (batch_size, 18, 1, 1) for tps, (batch_size, 6) for affine
        batch_size = theta_aff_tps_AB.size()[0]
        # P.shape: (batch_size, 2, 400)
        P = self.P.expand(batch_size, 2, self.N)
        # compute transformed grid points using estimated and GT tnfs
        # P_prime and P_prime_GT.shape: (batch_size, 2, 400)
        P_prime = self.pointTnf.tpsPointTnf(theta_aff_tps_AB.unsqueeze(2).unsqueeze(3), P)
        P_prime = self.pointTnf.affPointTnf(theta_aff_AB, P_prime)
        P_prime = self.pointTnf.tpsPointTnf(theta_aff_tps_BA.unsqueeze(2).unsqueeze(3), P_prime)
        P_prime = self.pointTnf.affPointTnf(theta_aff_BA, P_prime)
        # compute MSE loss on transformed grid points
        loss = torch.sum(torch.pow(P_prime - P, 2), 1)
        loss = torch.mean(loss)
        return loss

class InlierCount(nn.Module):
    def __init__(self, geometric_model='affine', tps_grid_size=3, tps_reg_factor=0, h_matches=15, w_matches=15,
                 use_conv_filter=False, dilation_filter=None, use_cuda=True, normalize_inlier_count=False,
                 offset_factor=227 / 210):
        super(InlierCount, self).__init__()
        self.normalize = normalize_inlier_count
        self.geometric_model = geometric_model
        self.geometricTnf = GeometricTnf(geometric_model=geometric_model, tps_grid_size=tps_grid_size,
                                         tps_reg_factor=tps_reg_factor, out_h=h_matches, out_w=w_matches,
                                         offset_factor=offset_factor, use_cuda=use_cuda)
        # define dilation filter
        # Generate binary structure for 8 neighbours
        ...
        if not use_conv_filter:
            ...
            for i in range(mask_id.shape[2]):
                flt = np.array([[1 / 16, 1 / 8, 1 / 16],
                                [1 / 8, 1 / 4, 1 / 8],
                                [1 / 16, 1 / 8, 1 / 16]])
                mask_id[:, :, i] = scipy.signal.convolve2d(mask_id[:, :, i], flt, mode='same', boundary='fill',
                                                           fillvalue=0)

        # convert to PyTorch variable
        mask_id = torch.FloatTensor(mask_id).permute(2, 0, 1).unsqueeze(0)
        mask_id.requires_grad = False
    ...
    def forward(self, theta, matches, return_outliers=False):
        # handle normal batch transformations
...
class DualInlierCount(InlierCount):
    def __init__(self, tps_grid_size=3, tps_reg_factor=0, h_matches=15, w_matches=15, use_conv_filter=False,
                 dilation_filter=None, use_cuda=True, normalize_inlier_count=False, offset_factor=227 / 210):
        super(DualInlierCount, self).__init__(h_matches=h_matches, w_matches=w_matches, use_conv_filter=use_conv_filter,
                                              dilation_filter=dilation_filter, use_cuda=use_cuda,
                                              normalize_inlier_count=normalize_inlier_count, offset_factor=offset_factor)

        self.compGeometricTnf = ComposedGeometricTnf(tps_grid_size=tps_grid_size, tps_reg_factor=tps_reg_factor,
                                                     out_h=h_matches, out_w=w_matches, offset_factor=offset_factor,
                                                     use_cuda=use_cuda)

    def forward(self, theta_aff, theta_aff_tps, matches, return_outliers=False):
        batch_size = theta_aff.size()[0]

        mask = self.compGeometricTnf(image_batch=expand_dim(self.mask_id, 0, batch_size), theta_aff=theta_aff,
                                     theta_aff_tps=theta_aff_tps)
        if return_outliers:
            mask_outliers = self.compGeometricTnf(image_batch=expand_dim(1.0 - self.mask_id, 0, batch_size),
                                                  theta_aff=theta_aff, theta_aff_tps=theta_aff_tps)

        if self.normalize:
            epsilon = 1e-5
            mask = torch.div(mask, torch.sum(torch.sum(torch.sum(mask + epsilon, 3), 2), 1).unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(mask))
            if return_outliers:
                mask_outliers = torch.div(mask, torch.sum(torch.sum(torch.sum(mask_outliers + epsilon, 3), 2), 1).unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(mask_outliers))
