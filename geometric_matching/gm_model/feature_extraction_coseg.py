...
class FeatureExtraction(nn.Module):
    ...
    def __init__(self, feature_extraction_cnn='resnet101', normalization=True, last_layer='', fixed_blocks=3, pytorch=False, caffe=False):
        ...
        if feature_extraction_cnn == 'resnet101':
            ...
            self.GM_base_1 = nn.Sequential(*resnet_module_list[:3])  # Get the feature maps of resnet101
            self.GM_base_2 = nn.Sequential(*resnet_module_list[3:5])  # Get the feature maps of resnet101
            self.GM_base_3 = nn.Sequential(*resnet_module_list[5])  # Get the feature maps of resnet101
            self.GM_base_4 = nn.Sequential(*resnet_module_list[6])  # Get the feature maps of resnet101
    ...
    def forward(self, image_batch):
        ...
        features_3 = self.GM_base_3(features_2)
        features_4 = self.GM_base_4(features_3)
        # Normalize feature maps
        if self.normalization:
            # features_1 = FeatureL2Norm(features_1)
            # features_2 = FeatureL2Norm(features_2)
            # features_3 = FeatureL2Norm(features_3)
            features_4 = FeatureL2Norm(features_4)
