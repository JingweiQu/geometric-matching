...
class AffineTheta(object):
    ...
    def __init__(self, use_cuda=True, image_size=240):
        self.use_cuda = use_cuda
        self.image_size = image_size

    def __call__(self, boxes_s=None, boxes_t=None, source_im_size=None, target_im_size=None):
        batch_size = boxes_s.shape[0]

        if source_im_size == None and target_im_size == None:
            source_im_size = torch.Tensor(batch_size, 3)
            source_im_size[:, 0:2] = self.image_size
            source_im_size[:, 2] = 3
            target_im_size = source_im_size.clone()
        else:
            # Re-locate the bounding box of the object in the original image
            for i in range(batch_size):
                boxes_s[i, 0:4:2] *= (source_im_size[i, 1] / self.image_size)
                boxes_s[i, 1:5:2] *= (source_im_size[i, 0] / self.image_size)

                boxes_t[i, 0:4:2] *= (target_im_size[i, 1] / self.image_size)
                boxes_t[i, 1:5:2] *= (target_im_size[i, 0] / self.image_size)
        ...
        src_pts[:, :, 0] = boxes_s[:, 0:2]
        src_pts[:, 0, 1] = boxes_s[:, 0]
        src_pts[:, 1, 1] = boxes_s[:, 3]
        src_pts[:, :, 2] = boxes_s[:, 2:4]

        dst_pts[:, :, 0] = boxes_t[:, 0:2]
        dst_pts[:, 0, 1] = boxes_t[:, 0]
        dst_pts[:, 1, 1] = boxes_t[:, 3]
        dst_pts[:, :, 2] = boxes_t[:, 2:4]
        ...
        for i in range(batch_size):
            if torch.sum(boxes_s[i, 0:4]).item() > 0 and torch.sum(boxes_t[i, 0:4]).item() > 0:
                affine_mat = self.affine_theta(src_pts_norm[i], dst_pts_norm[i])
            # If no object bounding box is detected, return identity affine theta (i.e. no warping for source image)
            else:
                affine_mat = np.array([[1, 0, 0], [0, 1, 0]]).astype(np.float32)
