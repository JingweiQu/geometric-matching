...
from geometric_matching.gm_model.loss_tps import TransformedGridLoss, CycleLoss, JitterLoss
...
# from geometric_matching.util.train_fn_tps import train_fn
from geometric_matching.util.train_fn_tps_control import train_fn
from geometric_matching.util.test_fn_tps import test_fn
# from geometric_matching.util.vis_fn_tps import vis_fn
from geometric_matching.util.vis_fn_tps_control import vis_fn
from geometric_matching.util.test_watch_tps import test_watch
...
if __name__ == '__main__':
    ...
    if args.use_mse_loss:
        ...
        loss_jitter = JitterLoss(use_cuda=args.cuda)
    ...
    checkpoint_name = os.path.join(args.trained_models_dir, args.feature_extraction_cnn, 'jitter', 'gm' + checkpoint_suffix + '_cycle3new.pth.tar')
    print('Checkpoint saving name: {}'.format(checkpoint_name))

    # Set visualization
    vis = Visdom(env='TPSCycle3')
    ...
    with torch.no_grad():
        ...
        vis_fn(vis=vis, train_loss=train_loss, val_pck=val_pck, train_lr=train_lr, epoch=0, num_epochs=args.num_epochs,
               dataloader=dataloader_watch, theta=theta_watch, thetai=thetai_watch, results=results_watch,
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        train_loss[epoch-1], train_time[epoch-1] = train_fn(epoch=epoch, model=model, loss_fn=loss, loss_cycle_fn=loss_cycle,
                                                            loss_jitter_fn=loss_jitter, lambda_c=lambda_c, lambda_j=lambda_j,
        ...
        if epoch % 5 == 0 or epoch == 1:
            with torch.no_grad():
                ...
                vis_fn(vis=vis, train_loss=train_loss, val_pck=val_pck, train_lr=train_lr, epoch=epoch,
                       ...
                       results=results_watch, geometric_model=args.geometric_model, use_cuda=True)
