...
from geometric_matching.util.vis_fn_dual import *
...
if __name__ == '__main__':
    ...
    if args.model != '':
        ...
        # GM_checkpoint['state_dict'] = OrderedDict(
        #     [(k.replace('FeatureRegression2', 'ThetaRegression2'), v) for k, v in GM_checkpoint['state_dict'].items()])
        ...
        #     model.ThetaRegression2.state_dict()[name].copy_(GM_checkpoint['state_dict']['ThetaRegression2.' + name])
    ...
    # Set visualization
    # vis = Visdom(env='test-aff&tps')
    model.eval()
    with torch.no_grad():
        # vis_fn_dual(vis=vis, model=model, train_loss=None, val_pck=None, train_lr=None, epoch=0, num_epochs=20, dataloader=dataloader_watch, use_cuda=args.cuda)
