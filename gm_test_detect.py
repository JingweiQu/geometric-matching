...
from geometric_matching.util.vis_fn_detect import *
...
if __name__ == '__main__':
    ...
    dataset_watch = WatchDataset(csv_file=csv_file_watch, dataset_path=args.test_dataset_path, output_size=output_size, normalize=normalize)
    ...
    # Set visualization
    # vis = Visdom(env='test-det&aff&tps')
    FasterRCNN.eval()
    model.eval()
    with torch.no_grad():
        # vis_fn_detect(vis=vis, model=model, faster_rcnn=FasterRCNN, aff_theta=AffTheta, train_loss=None,
        #               val_pck=None, train_lr=None, epoch=0, num_epochs=20, dataloader=dataloader_watch, use_cuda=args.cuda)
        results, _ = test_fn(model=model, faster_rcnn=FasterRCNN, aff_theta=AffTheta, metric=metric, dataset=dataset, dataloader=dataloader, detect=True, args=args)
