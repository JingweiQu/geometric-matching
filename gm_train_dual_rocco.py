...
from geometric_matching.gm_model.loss_rocco import TransformedGridLoss
...
from geometric_matching.util.train_fn_dual_rocco import train_fn_dual_rocco
...
if __name__ == '__main__':
    ...
    checkpoint_name = os.path.join(args.trained_models_dir, args.feature_extraction_cnn, 'rocco_gm' + checkpoint_suffix + '.pth.tar')
    print('Checkpoint saving name: {}'.format(checkpoint_name))

    # Set visualization
    vis = Visdom(env='RoccoAffine&TPSFinetuneOnRocco')
    ...
    with torch.no_grad():
        ...
        vis_fn_dual(vis=vis, train_loss=train_loss, val_pck=val_pck, train_lr=train_lr, epoch=0,
                    ...
                    theta_inver=theta_watch_inver, results=results_watch, use_cuda=args.cuda)
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        train_loss[epoch-1], train_time[epoch-1] = train_fn_dual_rocco(epoch=epoch, model=model, loss_fn=loss, optimizer=optimizer,
                                                                 dataloader=dataloader, triple_generation=triple_generation,
                                                                 use_cuda=args.cuda, log_interval=200, vis=vis)
        # model.eval()
        model.ThetaRegression.eval()
        model.ThetaRegression2.eval()
        results, val_time[epoch-1] = test_fn(model=model, metric='pck', batch_size=args.batch_size, dataset=dataset_val, dataloader=dataloader_val,
                                             dual=True, args=args)
