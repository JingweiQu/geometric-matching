# ========================================================================================
# Train geometric matching model
# Author: Jingwei Qu
...
from geometric_matching.gm_model.loss_dual import CycleLoss, InlierCount, DualInlierCount
...
from geometric_matching.util.train_fn_dual_cycle import train_fn_dual
...
if __name__ == '__main__':
    ...
    inliersAffine = InlierCount(geometric_model='affine', **arg_groups['weak_loss'])
    inliersTps = InlierCount(geometric_model='tps', **arg_groups['weak_loss'])
    inliersComposed = DualInlierCount(use_cuda=args.cuda, **arg_groups['weak_loss'])
    loss_cycle = CycleLoss(use_cuda=args.cuda)
    ...
    with torch.no_grad():
        ...
        vis_fn_dual(vis=vis, train_loss=train_loss, val_pck=val_pck, train_lr=train_lr, epoch=0,
                    ...
                    results=results_watch, title='AffTPSCycle', use_cuda=args.cuda)
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        train_loss[epoch-1], train_time[epoch-1] = train_fn_dual(epoch=epoch, model=model, loss_fn=loss,
                                                                 loss_cycle_fn=loss_cycle, optimizer=optimizer,
        ...
        if epoch % 5 == 0 or epoch == 1:
            with torch.no_grad():
                ...
                vis_fn_dual(vis=vis, train_loss=train_loss, val_pck=val_pck, train_lr=train_lr, epoch=epoch,
                            ...
                            thetai=thetai_watch, results=results_watch, title='AffTPSCycle', use_cuda=args.cuda)
